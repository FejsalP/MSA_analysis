...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput(inputId='reference_groups', label='Choose the number of genes per group fro reference list', value = 30, min = 1, max = 1000),
...
server <- function (input, output) {  #Load .clustal file and store it into start_stop reactive expression
  ...
  summary1 <- eventReactive(input$processButton, {
    ...
    variables$reference_list_separated <- create_reference_list_separated(coding_sequence_index, noncoding_sequence_index, variables$reference_list$reference_list)
